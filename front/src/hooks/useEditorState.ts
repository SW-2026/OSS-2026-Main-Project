...
export function useEditorState() {
  ...
  const handleGenerate = useCallback((prompt: string) => {
    ...
    const seq = Date.now();
    // 프롬프트에서 핵심 내용만 추출하고 웹툰 스타일 키워드 추가
    const cleanPrompt = prompt.replace(/,\s*/g, " ").replace(/\s+/g, " ").trim();
    const finalQuery = `${cleanPrompt} webtoon manga comic illustration anime art`;
    const imageUrl = `https://readdy.ai/api/search-image?query=$%7BencodeURIComponent%28finalQuery%29%7D&width=512&height=512&seq=${seq}&orientation=squarish`;
