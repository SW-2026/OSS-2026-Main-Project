...
export default function VectorEditor({
  ...
  // 경로 이동 상태
  const [draggingPath, setDraggingPath] = useState<{ pathId: string; startX: number; startY: number } | null>(null);
  ...
  const handleSVGMouseMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (draggingPath) {
        const path = paths.find((p) => p.id === draggingPath.pathId);
        if (!path) return;
        const dx = cx - draggingPath.startX;
        const dy = cy - draggingPath.startY;
        // 이동 후 초기화하기 위해 offset만 기록하지 않고 전체 재계산
      }
    },
  ...
  const handlePathMouseDown = useCallback(
    (e: React.MouseEvent, pathId: string) => {
      ...
      setDraggingPath({ pathId, startX: x, startY: y });
  ...
  const handleSVGMouseUp = useCallback(() => {
    ...
    if (draggingPath) {
      ...
      if (path && svgRef.current) {
        // 드래그 끝날 때 실제 이동량 계산해서 이동
        // 이미 move 동안 updatePath 호출됨
      }
      setDraggingPath(null);
    }
  }, [draggingNode, draggingPath, paths]);
